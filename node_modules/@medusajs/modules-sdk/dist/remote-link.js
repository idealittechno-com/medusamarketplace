...
class RemoteLink {
    ...
    async create(link) {
        ...
        for (const rel of allLinks) {
            ...
            if (!service) {
                throw new Error(`Module to link ${moduleA}[${moduleAKey}] and ${moduleB}[${moduleBKey}] was not found.`);
            }
    ...
    async dismiss(link) {
        ...
        for (const rel of allLinks) {
            ...
            const service = this.getLinkModule(moduleA, moduleAKey, moduleB, moduleBKey);
            if (!service) {
                throw new Error(`Module to dismiss link ${moduleA}[${moduleAKey}] and ${moduleB}[${moduleBKey}] was not found.`);
            }
            else if (!serviceLinks.has(service.__definition.key)) {
                serviceLinks.set(service.__definition.key, []);
            }
